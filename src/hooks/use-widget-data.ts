...
import type { Transaction, Category, SubCategory, Widget, Formula } from '@/types';
...
const aggregateMonthlyData = (
    ...
    const monthlyData = transactions.reduce((acc: { [key:string]: any }, transaction) => {
        try {
...
export function useWidgetData(
    ...
    const getWidgetData = useCallback((widget: Widget) => {
        const transactions = filterTransactions(allTransactions, globalFilters, widget);
        const kpis = calculateKpis(transactions, globalFilters, getBudgetDetails);
                ...
                try {
                    ...
                    formulaVariables.forEach(varName => {
                        if (!(varName in kpis)) {
                            ...
                            kpis[varName as keyof typeof kpis] = 0;
                        }
                    });
        ...
        const { monthlyData, keyMapping } = aggregateMonthlyData(transactions, widget);
    ...
    }, [allTransactions, globalFilters, formulas, getBudgetDetails]);
